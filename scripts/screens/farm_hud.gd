...
func _ready() -> void:
	...
	_status = make_label(top, "", 18, ACCENT)
...
func _refresh() -> void:
	var weather_txt := WeatherManager.current
	_status.text = "Day %d (%s) — %s | Weather: %s | Cash: $%d | Debt: $%d | Eggs: %d" % [
		CalendarManager.day, CalendarManager.weekday_name(), CalendarManager.block_name(),
		weather_txt, GameState.cash, GameState.debt, GameState.inventory.get("eggs", 0),
	]
	_rebuild_fields()
	_rebuild_info()
...
func _rebuild_fields() -> void:
	...
	for field_id in GameState.fields.keys():
		...
		var state_txt: String = "%s field: %s %s" % [field_id.capitalize(), f.state, f.crop]
...
func _rebuild_info() -> void:
	...
	match GameState.background_id:
		"old_school":
			...
			make_label(_info_col, WeatherManager.intuition_cue(), 14)
		...
		"it_nephew":
			...
			make_label(_info_col, "Forecast: %s" % " / ".join(fc), 14)

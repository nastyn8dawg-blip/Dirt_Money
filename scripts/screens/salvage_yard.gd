...
func _refresh() -> void:
	...
	for p in GameState.salvage_projects:
		...
		if p.hidden_hit:
			make_label(col, "   Found the painted-over problem. Parts and time both got worse.", 13, Color(0.9, 0.6, 0.5))
	...
	if GameState.salvage_offers.is_empty() and GameState.salvage_projects.is_empty():
		make_label(_list, "Nothing worth saving on the lot right now.", 14, Color(0.6, 0.6, 0.6))
	for o in GameState.salvage_offers:
		...
		for read in deal.get("condition_reads", []):
			make_label(col, "   \"%s\"" % read, 13, Color(0.75, 0.7, 0.6))
		if GameState.background_id == "mechanic":
			...
			], 13, Color(0.6, 0.85, 0.6))
		else:
			make_label(col, "   (Looks like a machine. Probably has parts in it.)", 12, Color(0.5, 0.5, 0.5))
		...
		make_button(row, "Buy it — $%d" % deal.buy_price, func():
			...
			_refresh())
		make_label(row, "or walk away — that's a skill too", 12, Color(0.5, 0.5, 0.5))
